...
describe("game1", function () {
  ...
  it("EVERYONE ALL IN AND GUY OVER BET MASSIVE", function (done) {
    ...
    let kim = new player(
      "5e83aa2c8391902cc37073b9",
      40000,
      "playing",
      "sid",
      "kim"
    );
    let shane = new player(
      "5e83aa9a64ca552d1437f9f3",
      1000000,
      "playing",
      "sid",
      "shane"
    ...
    let kev = new player(
      "5e83ab2844d4db2d4038c095",
      10000,
      "playing",
      "sid",
      "kev"
    );
    let clint = new player(
      "5e83ad4c4e56222def2e3148",
      100000,
      "playing",
      "sid",
      "clint"
  ...
  it("fold out on blinds", function (done) {
    ...
    game1.printSeats();
    chai.assert.equal(game1.isSettled(), true);
    chai.assert.equal(mike.balance + kev.balance, 29000 + 40000);
    done();
  });
  ...
  it("fold to an all in caller DOES IT END WITH MIKE CALLING THO?", function (done) {
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
        ...
        chai.assert.equal(
          mike.balance + kev.balance + shane.balance,
          10000 * 3
        );
  ...
  it("short stack allin is shoved on top of by a big stack all-iner", function (done) {
    ...
    setTimeout(function () {
      ...
      chai.assert.equal(mike.balance + kev.balance, 10500 + 9200);
      chai.assert.equal(game1.isSettled(), true);
      done();
    }, 7000);
  });
  ...
  it("boring check game", function (done) {
    ...
    setTimeout(function () {
      chai.assert.equal(game1.isSettled(), true);
      chai.assert.equal(mike.balance + kev.balance + shane.balance, 10000 * 3);
      done();
    }, 7000);
  });
  ...
  it("call clock", function (done) {
    ...
    setTimeout(function () {
      ...
      chai.assert.equal(mike.balance + kev.balance, 10000 + 5000);
  ...
  it("ensureing fold  on irver works due to a bug", function (done) {
    ...
    setTimeout(function () {
      ...
      chai.assert.equal(mike.balance + kev.balance, 4200 + 5000);
